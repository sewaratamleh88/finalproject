...
export function parseDateYmd(dateStr: string): Date | null {
  if (!DATE_YMD_RE.test(dateStr)) return null;
...
export function isPastDateYmd(dateStr: string): boolean {
...
export function validateOptionalString(
  ...
  fieldLabel: string,
  opts: { max?: number } = {},
): { error: string | null; value: string | undefined } {
  if (value === undefined) return { error: null, value: undefined };
  if (value === null) return { error: `${fieldLabel} must be a string`, value: undefined };
  if (typeof value !== 'string') return { error: `${fieldLabel} must be a string`, value: undefined };
  const trimmed = value.trim();
...
export function validateEmail(value: unknown): { error: string | null; email: string | null } {
  const requiredError = validateRequiredString(value, 'Email', { min: 3, max: LIMITS.email });
  if (requiredError) return { error: requiredError, email: null };
...
export function validateTaskPriority(value: unknown): string | null {
...
export function validateDateYmd(value: unknown, fieldLabel = 'Date'): string | null {
  if (value === undefined || value === null) {
    return `${fieldLabel} is required`;
  }
  if (typeof value !== 'string') {
...
export function validateOptionalDateYmd(value: unknown, fieldLabel = 'Date'): string | null {
  if (value === undefined) return null;
  if (value === null) return `${fieldLabel} must be a string`;
  if (typeof value !== 'string') return `${fieldLabel} must be a string`;
  const trimmed = value.trim();
  if (!trimmed) return `${fieldLabel} cannot be empty`;
  if (!parseDateYmd(trimmed)) {
    return `${fieldLabel} must be in YYYY-MM-DD format`;
  }
  return null;
...
export function validateOptionalTimeHm(value: unknown): string | null {
  if (value === undefined) return null;
  if (value === null) return 'Time must be a string';
